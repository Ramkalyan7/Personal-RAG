...
def stream_query_project(
    ...
    def event_generator():
        ...
        yield sse("message_start", {"id": stream_id})

        chunks: list[str] = []
        try:
            for delta in stream_rag_answer(question=payload.question, contexts=contexts):
                if not delta:
                    continue
                chunks.append(delta)
                yield sse("message_delta", {"id": stream_id, "delta": delta})
            ...
            yield sse("message_done", {"id": stream_id})
        except Exception as e:
            yield sse("error", {"id": stream_id, "message": str(e) or "Unknown error"})
